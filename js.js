...
let item_descr = document.querySelectorAll('.merch-item-descr')
let item_descr_all = ['Тарілка з принтом Марса. Доступна в червоному та білому кольорах', 'Кружка з принтом Марса. Доступна в червоному та білому кольорах', 'Масштабна модель однієї з ракет Galaxy', 'Масштабна модель марсохода (ми купуємо їх у NASA)']
for (let i = 0; i < item_descr_all.length; i += 1){
    item_descr[i].innerHTML = item_descr_all[i]
}
let item_price_objects = document.querySelectorAll('.merch-item-price')
let items_prices = ['1000', '1000', '2500', '5000']
for (let i = 0; i < item_price_objects.length; i += 1){
    item_price_objects[i].innerHTML = items_prices[i]
}

let cart_amount = document.querySelector('.cart-amount')
let count = Number(cart_amount.innerHTML)
